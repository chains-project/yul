...
- The `chains-hooks` marketplace source for `yul` is unpinned, but Claude Code doesn't re-resolve a plugin's source on every session — it reuses whatever it last cloned into `~/.claude/plugins/cache/<marketplace>/yul/`. So a user can end up running a stale plugin version indefinitely with no signal that a newer one exists. `ensure-yul.sh` also checks GitHub's latest release tag against the plugin version it resolved this session and, if newer, prints `SessionStart` hook JSON telling Claude to let the user know and suggest `/plugin marketplace update chains-project` (or reinstalling the plugin); this check fails open like the rest of the script.
