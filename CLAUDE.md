...
For `pom.xml`, the hook additionally blocks a newly added `<dependency>` whose `groupId:artifactId` — or whose exact pinned `<version>` — **doesn't exist on Maven Central at all**, a coordinate the model hallucinated. `RegistryExistenceChecker` (`pkg/maven/existence.go`) decides this through `git-pkgs/enrichment`'s direct-registry client, which for Maven queries Central's Solr endpoint (`search.maven.org`, `core=gav`) via `git-pkgs/registries`. Versions v0.8.1 and earlier of that package decoded the wrong JSON field for that response shape, so every returned version came back as an empty string — fixed upstream in [git-pkgs/registries#82](https://github.com/git-pkgs/registries/pull/82) (released as v0.9.1, which `go.mod` now pins directly). What's left is Solr's own search-index lag: a coordinate or a just-published version can briefly come back not-found before the index catches up, which a hook that blocks writes can't afford to read as "hallucinated" — so `Existence` retries a `registries.ErrNotFound`, and separately a queried version missing from an otherwise non-empty list, `existenceRetries` (1) more time after a short backoff before trusting either as a real miss. Any other error is returned immediately (no retry) so the caller fails open. `mismatch.Kind` (`KindOutdated` / `KindMissingPackage` / `KindMissingVersion`) tags which problem each report is: package not found at all vs. a real package with an invented version, and `main.go` renders those two lines differently while still exiting 2. The check lives in `pkg/maven` behind an injected `ExistenceChecker`, mirroring how `githubactions.Checker` takes a separate `Sha` resolver; a nil `ExistenceChecker` leaves only the outdated-version comparison. `yul scan` picks this up for free (it runs each checker with `before=""`). `benchmark/hallucination/compare.go` is a standalone diagnostic (`//go:build ignore`) that queries a coordinate both via git-pkgs/registries and via direct HTTP against `repo1.maven.org`'s static CDN, for comparing the two approaches' consistency.
